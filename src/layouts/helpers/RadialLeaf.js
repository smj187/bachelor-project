...
class RadialLeaf {
  ...
  render(isReRender) {
    ...
    for (let i = 0; i < nodeSize; i += 1) {
      ...
      const startingPoint = isRootLeaf === true ? helperCircle.pointAt(intervalSpaceUsed) : helperLine.pointAt(intervalSpaceUsed)
      intervalSpaceUsed += interval / 2
    ...
    if (isRootLeaf === false) {
      ...
      svg
        .attr({ opacity: isReRender ? 1 : 0 })
        .center(isReRender ? finalX : startX, isReRender ? finalY : startY)

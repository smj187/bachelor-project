import { SVG, Rect, ForeignObject, Image } from "../../../node_modules/@svgdotjs/svg.js/dist/svg.esm"
...
before(() => {
    ...
    cy.fixture("custom").then((databaseEntry) => {
        data = databaseEntry
        min = new Custom(databaseEntry, canvas)
        max = new Custom(databaseEntry, canvas)
    })
})


describe("custom node test", () => {
    it("renderAsMin()", () => {
        ...
        expect(min.getCurrentWidth()).to.equal(200)
        expect(min.getCurrentHeight()).to.equal(100)
        ...
        it("renderAsMax()", () => {
            ...
            expect(max.getCurrentWidth()).to.equal(275)
            expect(max.getCurrentHeight()).to.equal(175)
            expect(max.getCurrentX()).to.equal(600)
            expect(max.getCurrentY()).to.equal(250)

            // svg
            expect(max.getSVG().children().length).to.equal(3)
            expect(max.getSVG().children()[0] instanceof Rect).to.equal(true)
            expect(max.getSVG().children()[1] instanceof Image).to.equal(true)
            expect(max.getSVG().children()[2] instanceof ForeignObject).to.equal(true)
        }),

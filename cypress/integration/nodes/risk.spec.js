...
before(() => {
    ...
    cy.fixture("risk").then((databaseEntry) => {
        data = databaseEntry
        min = new Risk(databaseEntry, canvas)
        max = new Risk(databaseEntry, canvas)
    })
})


describe("risk node test", () => {
    it("renderAsMin()", () => {
        ...
        it("renderAsMax()", () => {
            ...
            expect(max.getCurrentWidth()).to.equal(300)
            expect(max.getCurrentHeight()).to.equal(150)
